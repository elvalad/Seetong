...
/**
 * Created by Administrator on 2014-10-08.
 */
...
//给控件设置监听事件
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        surface.width = ((View)this.getParent()).getWidth();//getResources().getDisplayMetrics().widthPixels;
        surface.height = ((View)this.getParent()).getHeight();//(int) (getResources().getDisplayMetrics().heightPixels * 2 / 5);
        final int widthMode = MeasureSpec.getMode(widthMeasureSpec);
        final int widthSize = MeasureSpec.getSize(widthMeasureSpec);
        final int heightMode = MeasureSpec.getMode(heightMeasureSpec);
        final int heightSize = MeasureSpec.getSize(heightMeasureSpec);
        /*if (widthMode == View.MeasureSpec.EXACTLY || widthMode == MeasureSpec.AT_MOST) {
            surface.width = widthSize;
        }
        if (heightMode == View.MeasureSpec.EXACTLY || heightMode == MeasureSpec.AT_MOST) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            // Measure the text
            surface.width = getResources().getDisplayMetrics().widthPixels - (getPaddingLeft() + getPaddingRight());
            if (widthMode == MeasureSpec.AT_MOST) {
                // Respect AT_MOST value if that was what is called for by measureSpec
                surface.width = Math.min(surface.width, widthSize);
            }
        }
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            // Measure the text
            surface.height = getResources().getDisplayMetrics().heightPixels / 2;
            if (heightMode == MeasureSpec.AT_MOST) {
                // Respect AT_MOST value if that was what is called for by measureSpec
                surface.height = Math.min(surface.height, heightSize);
            }
        }
...
//      }
        ...
        if (y > surface.monthHeight + surface.weekHeight) {
            ...
            int n = (int) (Math.floor((y - (surface.monthHeight + surface.weekHeight)) / surface.cellHeight) + 1);
            downIndex = (n - 1) * 7 + m - 1;
...
//              }
                    selectedStartDate = selectedEndDate = downDate;
                    //响应监听事件
                    if (onItemClickListener != null) onItemClickListener.OnItemClick(selectedStartDate);
